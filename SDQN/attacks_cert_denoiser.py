...
def pgd(model, X, y, epsilon=0, verbose=False, params={}, env_id="", norm_type='l_2'):
    ...
    for i in range(niters):
        # print('iter', i, 'X_adv', X_adv)
        logits = model.noised_forward(X_adv)
        #logits = model.forward(X_adv)
        # print('iter', i, 'logits', logits)
...
def attack(model, X, attack_config, epsilon, loss_func=nn.CrossEntropyLoss()):
    ...
    if method == 'cw':
        ...
        atk = pgd
    adv_X = atk(model, X, y, epsilon=epsilon, verbose=verbose, params=params)
