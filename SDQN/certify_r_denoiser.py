...
from attacks_cert_denoiser import attack
...
def main(args):
    ...
    for frame_idx in range(1, num_episodes * max_frames_per_episode + 1):
        ...
        for i in range(len(e)):
            per_state_tensor = attack(model, state_tensor, attack_config, e[i])
            eps_list[i].append(torch.norm(per_state_tensor-state_tensor).cpu())
        ...
        if done:
            ...
            # store
            certify_file = test_log+f'_R-list-{episode_idx}.pt'
            torch.save(model.R_list, certify_file)
            for i in range(len(e)):
                eps_file = test_log+f'_eps-{e[i]}list-{episode_idx}.pt'
                torch.save(eps_list[i], eps_file)
            logger.log(f'certified result saved to {certify_file}')
            radius = []
            for t in model.R_list:
                t = list(t)
                if t[2] < 0:
                    t[2] = 0
                radius.append(t[2])
            print("avg certify radius:", np.mean(np.asarray(radius)))
