...
from paad_rl.a2c_ppo_acktr.algo.imitation import DataBuffer
...
def imitate_update(agent, expert_data, device):
    ...
    loss = ce_loss(action_dist, expert_action.squeeze(1))
...
def main():
    ...
    model_save_path = os.path.join(save_path, exp_name + name_suffix + "_attacker")
    ...
    if "denoiser" in args.config:
        ...
        denoise = False
    model = model_setup(env_id, envs, use_cuda=True, dueling=True, model_width=1, m=1, sigma=sigma, denoise=denoise)
    if denoise:
...
#                 extra_reward = 0
        ...
        if not args.test:
            torch.save(actor_critic, os.path.join(save_path, exp_name + name_suffix + "_attacker_final"))
