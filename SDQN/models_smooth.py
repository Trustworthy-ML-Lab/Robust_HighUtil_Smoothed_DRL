...
class QNetwork(nn.Module):
    ...
    def noised_forward(self, x):
        if self.sigma!=0.0:
            x_noise = x + torch.FloatTensor(*self.input_shape).normal_(0, self.sigma).cuda()
        else:
            x_noise = x
        value = self.features(x_noise)
        return value

    def forward(self, *args, **kwargs):
        # print(*args)
        value = self.features(*args, **kwargs)
        # print(value)
    ...
    def act(self, state, epsilon=0, perturb=-1, return_q=False, cert=True):
        ...
        with torch.no_grad():
            if self.sigma!=0.0:
                ...
                self.noise_list = [torch.zeros(*self.input_shape).cuda()
                               for _ in range(self.m)]
