...
def attack(denoiser, model, X, attack_config, sigma_schedule, loss_func=nn.CrossEntropyLoss()):
    norm_type = attack_config.get('norm_type', 'l_inf')
    params = attack_config.get('params', {})
    params['loss_func'] = loss_func
    im = denoiser(X, sigma_schedule=sigma_schedule)
    y = model.act(im)
    ...
    for i in range(niters):
        im = denoiser(X_adv, sigma_schedule=sigma_schedule)
        logits = model(im)
...
def compute_td_loss(current_denoiser, target_denoiser, current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, a_dim, **kwargs):
    ...
    attack_config = kwargs['attack_config']

    if random.random() > 0.7:
        state_perturbed, _ = attack(current_denoiser, current_model, state, attack_config, sigma_schedule)
    else:
        state_perturbed = state

    im = current_denoiser(state_perturbed, sigma_schedule=sigma_schedule)
...
def main(args):
    ...
    for frame_idx in range(load_frame, num_frames + 1):
        ...
        if random.random() > 0.7:
            ...
            state_tensor, epsilon = attack(current_denoiser, current_model, state_tensor, attack_config, sigma_schedule)
        ...
        if frame_idx % training_config['print_frame'] == 0 or frame_idx==num_frames or abs(buffer_stored_size-replay_initial) < 5:
            ...
            logger.log('sigma:{:.6g}, epsilon:{:.6g}'.format(sigma_schedule, epsilon))
