...
def ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, 
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            if params.SMOOTHING:
                raise Exception("SPPO not implemented for LSTM policy yet")
            # LSTM policy. Need to go over all episodes instead of states.
            ...
            for selected in splits:
                ...
                # dist contains mean and variance of Gaussian.
                if params.SMOOTHING:
                    num = params.TRAINING_M
                    total_action_mean = []
                    total_action_std = []
                    for i in range(num):
                        noised_states = batch_states + ch.normal(0.0, params.SIGMA, batch_states.size())
                        action_pds = net(noised_states)
                        total_action_mean.append(action_pds[0])
                        total_action_std.append(action_pds[1])
                    dist = (ch.median(ch.stack(total_action_mean), dim=0)[0],
                            ch.median(ch.stack(total_action_std), dim=0)[0])
                else:
...
"""Computing an estimated upper bound of KL divergence for policy std dependent on state."""
def get_state_kl_bound_stdv(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        if beta is None:
            ...
            ikl = intermediate_to_kl_stdv(ilb, iub, means, stdev=stdev)
            ckl = intermediate_to_kl_stdv(clb, cub, means, stdev=stdev)
            return ikl, ckl
        else:
            ...
            kl2 = intermediate_to_kl(lb[:,:6], ub[:,:6], means, stdev=stdev)
            # lb[:,6:] = stdev
            # ub[:,6:] = stdev
            kl = intermediate_to_kl_stdv(lb, ub, means, stdev=stdev)
            return kl


def intermediate_to_kl_stdv(lb, ub, means, stdev=None):
    batch_dim = lb.shape[0]
    act_dim = lb.shape[1]//2
    lb_mean, lb_std = lb[:,:act_dim], lb[:, act_dim:]
    ub_mean, ub_std = ub[:,:act_dim], ub[:, act_dim:]
    lb_var, ub_var, var = lb_std.pow(2), ub_std.pow(2), stdev.pow(2)

    ub_diff = means-ub_mean
    ub_log_quot_frac = ch.log(ub_var).sum() - ch.log(var).sum()
    ub_tr = (var / ub_var).sum()
    ub_quadratic = ((ub_diff / ub_var) * ub_diff).sum(dim=1)
    ub_kl_sum = 0.5 * (ub_log_quot_frac - act_dim*batch_dim + ub_tr + ub_quadratic)

    lb_diff = means-lb_mean
    lb_log_quot_frac = ch.log(var).sum() - ch.log(lb_var).sum()
    lb_tr = (var / lb_var).sum()
    lb_quadratic = ((lb_diff / lb_var) * lb_diff).sum(dim=1)
    lb_kl_sum = 0.5 * (lb_log_quot_frac - act_dim*batch_dim + lb_tr + lb_quadratic)

    return torch.max(ub_kl_sum.abs(), lb_kl_sum.abs())
...
"""Computing an estimated upper bound of KL divergence using ibp."""
def get_state_kl_bound_ibp(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
...
"""Computing an estimated upper bound of KL divergence using ibp."""
def get_state_kl_bound_ibp2(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        if beta is None:
            ...
            if 1 - beta < 1e-20:
                ...
                lb = beta * ilb + (1 - beta) * clb
                ub = beta * iub + (1 - beta) * cub
            diff = intermediate_to_kl(lb, ub, means, stdev=stdev)
...
def radial_ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, 
                advs, net, params, store, opt_step, relaxed_net, eps_scheduler, beta_scheduler):
    '''

    Proximal Policy Optimization with worst case perturbation in policy network
    Runs K epochs of PPO as in https://arxiv.org/abs/1707.06347
    Inputs:
    - all_states, the historical value of all the states
    - actions, the actions that the policy sampled
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            raise Exception("Radial PPO not implemented for LSTM policy yet")
            # LSTM policy. Need to go over all episodes instead of states.
            params.POLICY_ADAM.zero_grad()
            hidden = None
            surrogate = 0.0
            ...
            for selected in splits:
                ...
                current_eps = eps_scheduler.get_eps()
                # changed to stdev dependent on state
                stdev = ch.exp(net.log_stdev)
                # stdev = dist[1]
                if params.ROBUST_PPO_DETACH_STDEV:
                    # Detach stdev so that it won't be too large.
                    stdev = stdev.detach()
                if params.ROBUST_PPO_METHOD in ["convex-relax"]:
                    # get upper/lower bound of \mu
                    ub_mean, lb_mean = get_mean_bound(relaxed_net, batch_states, batch_action_means,
                            eps=current_eps, beta=beta_scheduler.get_eps(),
                            stdev=stdev)
                    # get upper/lower bound of log likelihood
                    ub_new_log_ps, lb_new_log_ps = get_loglikelihood_bound(mean, ub_mean,lb_mean, stdev, batch_actions)

                    ub_unclp_rew = surrogate_reward(batch_advs, new=ub_new_log_ps, old=batch_old_log_ps,normalize=False)
                    ub_clp_rew = surrogate_reward(batch_advs, new=ub_new_log_ps, old=batch_old_log_ps,
                                           clip_eps=params.CLIP_EPS,normalize=False)
                    lb_unclp_rew = surrogate_reward(batch_advs, new=lb_new_log_ps, old=batch_old_log_ps,normalize=False)
                    lb_clp_rew = surrogate_reward(batch_advs, new=lb_new_log_ps, old=batch_old_log_ps,
                                           clip_eps=params.CLIP_EPS,normalize=False)
                    ...
                    kl_upper_bound = get_state_kl_bound_sgld(net, batch_states, batch_action_means,
                            eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                            stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "ibp":
                    kl_upper_bound = get_state_kl_bound_ibp(relaxed_net, batch_states, batch_action_means,
                            eps=current_eps, beta=beta_scheduler.get_eps(),
                            stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "ibp2":
                    kl_upper_bound = get_state_kl_bound_ibp2(relaxed_net, batch_states, batch_action_means,
                            eps=current_eps, beta=beta_scheduler.get_eps(),
                            stdev=stdev).mean()
                else:
                    raise ValueError(f"Unsupported robust PPO method {params.ROBUST_PPO_METHOD}")
                ...
                # Total loss, is the min of clipped and unclipped reward for each state, averaged.
                surrogate_clean = ch.min(unclp_rew, clp_rew)
                # ,ub_unclp_rew, ub_clp_rew,lb_unclp_rew, lb_clp_rew]
                surrogate_adv = unclp_rew
                for tensor in [clp_rew,ub_unclp_rew, ub_clp_rew,lb_unclp_rew, lb_clp_rew]:
                    surrogate_adv = ch.min(surrogate_adv, tensor)
                # if sum(torch.abs(surrogate-surrogate_clean))>1e-4:
                    # print("debug")
                if current_eps > 0:
                    surrogate_ = 0.5*surrogate_adv+0.5*surrogate_clean
                else:
                    surrogate_ = surrogate_clean
                surrogate_adv_value = -surrogate_adv.mean()
                surrogate_clean_value = -surrogate_clean.mean()
                surrogate = -surrogate_.mean()
                entropy = -params.ENTROPY_COEFF * entropy_bonus
                # only surrogate and entropy
                # loss = surrogate + entropy + params.ROBUST_PPO_REG * kl_upper_bound
                loss = surrogate + entropy
                # If we are sharing weights, take the value step simultaneously 
                # (since the policy and value networks depend on the same weights)
        ...
        print(f'eps={eps_scheduler.get_eps():8.6f}, beta={beta_scheduler.get_eps():8.6f}, '
...
def get_loglikelihood_bound(mean,ub_mean, lb_mean,stdev, batch_actions):
    try:    
        # ub_diff = torch.min(torch.abs(ub_mean-batch_actions), torch.abs(lb_mean-batch_actions))
        ub_diff = torch.max(batch_actions-ub_mean, lb_mean-batch_actions)
        ub_diff = torch.max(ub_diff, torch.zeros_like(ub_mean))
        lb_diff = torch.max(torch.abs(ub_mean-batch_actions), torch.abs(lb_mean-batch_actions))
        # ub_diff = batch_actions-mean
        # lb_diff = batch_actions-mean

        ub_nll =  0.5 * ((ub_diff) / stdev).pow(2).sum(-1) \
                + 0.5 * np.log(2.0 * np.pi) * batch_actions.shape[-1] \
                + torch.log(stdev).sum(-1)
        lb_nll =  0.5 * ((lb_diff) / stdev).pow(2).sum(-1) \
                + 0.5 * np.log(2.0 * np.pi) * batch_actions.shape[-1] \
                + torch.log(stdev).sum(-1)
        return -ub_nll, -lb_nll
...
def robust_ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, 
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            if params.SMOOTHING:
                raise Exception("S-SAPPO not implemented for LSTM policy yet")
            # LSTM policy. Need to go over all episodes instead of states.
            params.POLICY_ADAM.zero_grad()
            hidden = None
            surrogate = 0.0
...
def robust_ppo_stdv_step(all_states, actions, old_log_ps, rewards, returns, not_dones, 
                advs, net, params, store, opt_step, relaxed_net, eps_scheduler, beta_scheduler):
    '''
    Proximal Policy Optimization with robustness regularizer
    Runs K epochs of PPO as in https://arxiv.org/abs/1707.06347
    ...
    '''
    if params.SMOOTHING:
        raise Exception("S-SAPPO-stdv not implemented yet")
    # Storing batches of stuff
    # if store is not None:
    #     orig_dists = net(all_states)
            ...
            for selected in splits:
                ...
                if params.ROBUST_PPO_METHOD in ["convex-relax"]:
                    kl_upper_bound = get_state_kl_bound(relaxed_net, batch_states, batch_action_means,
                            eps=current_eps, beta=beta_scheduler.get_eps(),
                            stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD in  ["convex-relax_stdv"]:
                    kl_upper_bound = get_state_kl_bound_stdv(relaxed_net, batch_states, batch_action_means,
                            eps=current_eps, beta=beta_scheduler.get_eps(),
                            stdev=stdev).mean()
...
def trpo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, advs, net, params, store, opt_step):
    ...
    '''    
    if params.SMOOTHING:
        raise Exception("STRPO not implemented yet")
    # Initial setup
...
def step_with_mode(mode, adversary=False):
    STEPS = {
        ...
        'radial_ppo': radial_ppo_step,
        'robust_ppo_stdv': robust_ppo_stdv_step,
        'adv_ppo': ppo_step,
        'adv_trpo': trpo_step,
