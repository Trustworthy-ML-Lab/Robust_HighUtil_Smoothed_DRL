...
class Trainer():
    ...
    def perturb_obs_fgsm(self, directions, last_states):
        ...
        if self.SMOOTHING:
            noised_states = last_states + ch.normal(0.0, self.SIGMA, last_states.size())
            old_action = self.policy_model(noised_states)[0].detach()
        else:
        ...
        with torch.enable_grad():
            if self.SMOOTHING:
                noised_states = last_states.detach() + ch.normal(0.0, self.SIGMA, last_states.size())
                new_action = self.policy_model(noised_states + noise)[0]
            else:
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None):
        ...
        with torch.no_grad():
            ...
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, ep_avg_adiv, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound
