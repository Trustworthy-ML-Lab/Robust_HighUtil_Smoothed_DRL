...
def main(params):
    ...
    if 'load_model' in params and params['load_model']:
        ...
        if 'load_q_model' in params and params['load_q_model']:
            q_model = torch.load(params['load_q_model'])
        ...
        if 'q_model' in pretrained_model and params['mode'] == 'robust_q_ppo':
            p.q_model.load_state_dict(pretrained_model['q_model'])
        if 'q_opt' in pretrained_model and params['mode'] == 'robust_q_ppo':
            p.q_opt.load_state_dict(pretrained_model['q_opt'])
        if 'load_q_model' in params and params['load_q_model']:
            p.q_model.load_state_dict(q_model['q_model'])
            p.q_opt.load_state_dict(q_model['q_opt'])
    ...
    if params['sarsa_enable']:
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, ep_avg_adiv, actions, action_means, states, kl_certificates, worst_q = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=original_stdev, compute_worst_q=params['compute_worst_q'])
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, ep_avg_adiv, actions, action_means, states, kl_certificates, worst_q, value = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=original_stdev, compute_worst_q=params['compute_worst_q'])
            if params['smoothing']:
                all_adiv.append(ep_avg_adiv)
            if i == 0:
                all_actions = actions.copy()
                all_states = states.copy()
            else:
            ...
            if params['compute_worst_q']:
                print('Epoch worst q:', worst_q)
                all_worst_q.append(worst_q)
                print('Epoch value:', value)
        ...
        if params['compute_worst_q']:
            print('Worst Q stats: mean: {}, std: {}, min: {}, max: {}'.format(np.mean(all_worst_q), np.std(all_worst_q), np.min(all_worst_q), np.max(all_worst_q)))
            print('Value stats: mean: {}, std: {}, min: {}, max: {}'.format(np.mean(all_value), np.std(all_value), np.min(all_value), np.max(all_value)))
...
def get_parser():
    ...
    parser.add_argument('--compute-worst-q', action='store_true', help='compute worst q')
    ...
    parser.add_argument('--load-q-model', type=str, help='load a pretrained worst-q model file', default='')
