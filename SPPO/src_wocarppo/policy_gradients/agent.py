...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        if self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_q_ppo':
        ...
        self.val_model = value_net_class(self.NUM_FEATURES, self.INITIALIZATION, activation=self.value_activation)
    ...
    def setup_stores(self, store):
        ...
        if self.params.MODE == 'robust_q_ppo':
            worst_q_cols ={
                ...
                'loss': float,
                'worst_q': float,
                'worst_q_weight': float,
                'current_value': float,
            }
            self.store.add_table('robust_q_ppo_data', worst_q_cols)
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        - action_log_probs: (# actors, self.T, ) log probabilities of each action
        ...
        # initial next states and states
        ...
        for t in iterator:
            ...
            if self.SMOOTHING:
                ...
                total_action_std = []
                for i in range(num):
                    noised_states = last_states + ch.normal(0.0, self.SIGMA, last_states.size())
                    action_pds = self.policy_model(noised_states)
                    total_action_mean.append(action_pds[0])
                    total_action_std.append(action_pds[1])
                action_pds = (ch.median(ch.stack(total_action_mean), dim=0)[0], ch.median(ch.stack(total_action_std), dim=0)[0])
            else:
        ...
        # next state is the states on next positions
        next_states = states[:,1:,:]
        states = states[:,:-1,:]
        trajs = Trajectories(rewards=rewards, 
            action_log_probs=action_log_probs, not_dones=not_dones, 
            actions=actions, states=states, next_states=next_states, action_means=action_means, action_std=next_action_stds)
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        if self.MODE == "robust_q_ppo": 
            if self.ANNEAL_LR:
                q_scheduler = self.Q_SCHEDULER
            q_model = self.q_model
            q_opt = self.q_opt
            target_q_model = self.target_q_model
        ...
        if self.MODE == 'robust_q_ppo':
            ...
            q_loss = worst_q_step(saps.states, saps.actions, saps.next_states, saps.not_dones, saps.rewards, q_model, target_q_model, policy_model, q_opt, self.params, self.robust_eps_scheduler)
            if self.ANNEAL_LR and increment_scheduler:
                q_scheduler.step()
            if logging:
                self.store.log_table_and_tb('optimization'+table_name_suffix, {
        ...
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_q_ppo') and not adversary_step and logging:
            # Logging Robust PPO KL, entropy, etc.
            store_to_pass = self.store

        # Take policy optimizer steps
        ...
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_q_ppo') and isinstance(self.policy_model, CtsPolicy) and not adversary_step:
        ...
        if adversary_step:
            ...
            if self.MODE == 'robust_q_ppo': 
                policy_loss, surr_loss, entropy_bonus = self.policy_step(*args, target_q_net=self.target_q_model, value_net=self.val_model, n_steps = self.n_steps)
            else:
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None, compute_worst_q= False):
        ...
        with torch.no_grad():
            ...
            ep_length, ep_reward, ep_avg_adiv, actions, action_means, states = output
            msg = "Episode reward: %f | episode adiv: %f | episode length: %f"
            print(msg % (ep_reward, ep_avg_adiv, ep_length))
            if compute_bounds:
            ...
            if compute_worst_q:
                value = self.val_model(states).mean()
                worst_q = self.q_model(torch.cat((states, action_means), dim=1)).mean()
                value = value.item()
                worst_q = worst_q.item()
            else:
                worst_q = float("nan")
                value = float("nan")
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, ep_avg_adiv, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound, worst_q, value
    ...
    def run_test_trajectories(self, max_len, should_tqdm=False, deterministic=False):
        ...
        for t in iterator:
            ...
            if self.SMOOTHING:
                ...
                total_action_std = []
                for i in range(num):
                    noised_state = maybe_attacked_last_states + ch.normal(0.0, self.SIGMA, maybe_attacked_last_states.size())
                    action_pds = self.policy_model(noised_state)
                    total_action_mean.append(action_pds[0])
                    total_action_std.append(action_pds[1])
                action_pds = (ch.median(ch.stack(total_action_mean), dim=0)[0], ch.median(ch.stack(total_action_std), dim=0)[0])
                p1, q1 = norm.cdf(norm.ppf(0.5-0.1224)-0.1/self.SIGMA), norm.cdf(norm.ppf(0.5+0.1224)+0.1/self.SIGMA)
                p2, q2 = norm.cdf(norm.ppf(0.5-0.1224)-0.2/self.SIGMA), norm.cdf(norm.ppf(0.5+0.1224)+0.2/self.SIGMA)
                p3, q3 = norm.cdf(norm.ppf(0.5-0.1224)-0.3/self.SIGMA), norm.cdf(norm.ppf(0.5+0.1224)+0.3/self.SIGMA)
                adiv1 = np.linalg.norm(np.quantile(ch.stack(total_action_mean).numpy(), q1, axis=0) - np.quantile(ch.stack(total_action_mean).numpy(), p1, axis=0))/(2 * 0.1)
                adiv2 = np.linalg.norm(np.quantile(ch.stack(total_action_mean).numpy(), q2, axis=0) - np.quantile(ch.stack(total_action_mean).numpy(), p2, axis=0))/(2 * 0.2)
                adiv3 = np.linalg.norm(np.quantile(ch.stack(total_action_mean).numpy(), q3, axis=0) - np.quantile(ch.stack(total_action_mean).numpy(), p3, axis=0))/(2 * 0.3)
                adiv = (adiv1 + adiv2 + adiv3) / 3
                total_adiv.append(adiv)
            else:
        ...
        if total_adiv:
            ep_avg_adiv = np.mean(total_adiv)
        else:
            ep_avg_adiv = float("nan")

        to_ret = (ep_length, ep_reward, ep_avg_adiv, actions, action_means, states)
