...
def main(params):
    ...
    if params['mode'] == "robust_q_ppo":
        worst_q_table_dict = {
            'q_model': store.PYTORCH_STATE,
            'q_opt': store.PYTORCH_STATE,
        }
    else:
        worst_q_table_dict = {}
    ...
    if 'load_model' in params and params['load_model']:
        ...
        if 'q_model' in pretrained_model:
            p.q_model.load_state_dict(pretrained_model['q_model'])
        if 'q_opt' in pretrained_model:
    ...
    final_dict.update(worst_q_table_dict)
    ...
    def add_q_to_table(p, table_dict):
        if params['mode'] == 'robust_q_ppo':
    ...
    def finalize_table(iteration, terminated_early, rewards):
        ...
        final_dict = add_q_to_table(p, final_dict)
    ...
    try:
        for i in range(params['train_steps']):
            print('Step %d' % (i,))
            if params['save_iters'] > 0 and i % params['save_iters'] == 0 and i != 0 and i > 0.9 * params['train_steps']:
                final_5_rewards = np.array(rewards)[-5:].mean()
                ...
                checkpoint_dict = add_q_to_table(p, checkpoint_dict)
                store['checkpoints'].append_row(checkpoint_dict)
                
                checkpoint_path = store.path + '/checkpoint'
                if not os.path.exists(checkpoint_path):
                    os.makedirs(checkpoint_path)
                print(f'Saving model to {store.path} with reward {final_5_rewards:.5g}')
                cp = {
                    'policy_model': p.policy_model.state_dict(),
                    'val_model': p.val_model.state_dict(),
                    'envs': p.envs
                }
                torch.save(cp, '{}/{}.model'.format(checkpoint_path, i))
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--smoothing', type=str2bool, help='smoothing')
    parser.add_argument('--training-m', type=int, help='training-m')
    parser.add_argument('--testing-m', type=int, help='testing-m')
    return parser
